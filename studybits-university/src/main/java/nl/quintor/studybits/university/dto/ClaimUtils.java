...
public class ClaimUtils {
    ...
    public static SchemaDefinition getSchemaDefinition(Class<?> claimClass) {
        validateClaimType(claimClass);
        SchemaInfo schemaInfo = claimClass.getAnnotation(SchemaInfo.class);
        List<String> fieldNames = getFieldNames(claimClass);
        return new SchemaDefinition(schemaInfo.name(), schemaInfo.version(), fieldNames);
    }

    public static String getSchemaName(Class<?> claimClass) {
        validateClaimType(claimClass);
        return claimClass.getAnnotation(SchemaInfo.class).name();
    }

    public static String getSchemaVersion(Class<?> claimClass) {
        validateClaimType(claimClass);
        return claimClass.getAnnotation(SchemaInfo.class).version();
    }

    public static Map<String, Object> getMapOfClaim(Object claim) {
        validateClaimType(claim.getClass());
        return Arrays.stream(claim.getClass().getDeclaredFields())
