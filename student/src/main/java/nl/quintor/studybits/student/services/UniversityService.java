...
public class UniversityService {
    ...
    public University createAndSave(String name, String endpoint) {
        if ( universityRepository.existsByName(name) ) throw new IllegalArgumentException("University with name exists already.");
    ...
    public void updateById( University university ) {
        if ( !universityRepository.existsById(university.getId()) ) throw new IllegalArgumentException("University with id not found.");

        universityRepository.save(university);
    }

    public void deleteById( Long uniId ) {
        if ( !universityRepository.existsById(uniId) ) throw new IllegalArgumentException("University with id not found.");
    ...
    private URI buildOnboardingUri( University university, String endpoint, Student student ) {
        return UriComponentsBuilder.fromPath(university.getEndpoint())
