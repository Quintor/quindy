...
public class Verifier extends WalletOwner {
    ...
    public CompletableFuture<Map<String, Map.Entry<String, String>>> verifyProof(ProofRequest proofRequest, Proof proof) {
        return getEntitiesFromLedger(proof.getIdentifiers())
                .thenCompose(wrapException(entitiesFromLedger -> Anoncreds.verifierVerifyProof(proofRequest.toJSON(), proof.toJSON(),
                        JSONUtil.mapper.writeValueAsString(entitiesFromLedger.getSchemas()),
                        JSONUtil.mapper.writeValueAsString(entitiesFromLedger.getClaimDefs()),"{}")))
                .thenApply(result -> {
                    if (!result) {
                        throw new IndyWrapperException("Invalid proof: verifierVerifyProof returned false");
                    }

                    boolean allEncodingsValid = proof.getRequestedProof().getRevealedAttributes().values().stream()
                    ...
                    Stream<Map.Entry<String, String>> selfAttestedStream = proof.getRequestedProof().getSelfAttestedAttributes()
                            .entrySet().stream();
                    Stream<Map.Entry<String, String>> revealedStream = proof.getRequestedProof().getRevealedAttributes().entrySet().stream()
                            .map(entry -> new AbstractMap.SimpleEntry<>(entry.getKey(), entry.getValue().get(1)));

                    Map<String, Map.Entry<String, String>> attributeValues = Stream.concat(selfAttestedStream, revealedStream)
                            .collect(Collectors.toMap(Map.Entry::getKey, entry ->
                                    new AbstractMap.SimpleEntry<>(proofRequest.getRequestedAttrs().get(entry.getKey()).getName(), entry.getValue())));
