...
public class UserContext {
    ...
    public void setCurrentUser( String universityName, String userName ) {
        if ( StringUtils.isNoneBlank(universityName, userName) ) {
            Optional<Long> userId = userRepository.findIdByUniversityNameAndUserName(universityName, userName);
    ...
    public UserIdentity currentUserIdentity() {
        return getCurrentUser().orElseThrow(() -> new IllegalArgumentException("UserIdentity information missing."));
    }


    public Long currentUserId() {
        return currentUserIdentity().getUserId()
