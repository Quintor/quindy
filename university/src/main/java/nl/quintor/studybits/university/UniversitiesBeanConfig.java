...
public class UniversitiesBeanConfig {
    ...
    private Issuer ensureIssuer( String universityName ) {
        return createIssuer(universityName).orElseGet(() -> getIssuer(universityName));
    }
    ...
    private Issuer onboardIssuer( IndyWallet wallet ) {
        ...
        ConnectionRequest connectionRequest = steward.createConnectionRequest(issuer.getName(), "TRUST_ANCHOR")
                                                     .get();
        AnoncryptedMessage newcomerConnectionResponse = issuer.acceptConnectionRequest(connectionRequest)
        ...
        steward.anonDecrypt(newcomerConnectionResponse, ConnectionResponse.class)
               ...
               .get();
        AuthcryptedMessage verinym = issuer.authcrypt(issuer.createVerinymRequest(connectionRequest.getDid()))
                                           .get();
