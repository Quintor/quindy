...
public class ProofRequestController {
    ...
    @Autowired
    public ProofRequestController(UserContext userContext, LinkHelper linkHelper, ProofService proofService, Mapper mapper) {
    ...
    List<ProofRequestInfo> findAllProofRequests() {
        return proofService.getProofHandlers()
                .values()
                .stream()
    ...
    AuthEncryptedMessageModel getProofRequest(@PathVariable String proofName, @PathVariable Long proofId) {
        ProofHandler handler = proofService.getHandler(proofName);
